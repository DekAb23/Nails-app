...
import { Instagram, Phone, Calendar, Check, X, Lock, Sparkles, ChevronLeft, RotateCcw } from 'lucide-react';
...
export default function Home() {
  ...
  const fetchMyAppointments = async (phone: string, skipVerification = false) => {
    ...
    if (!skipVerification && !isPhoneVerified(phoneDigits)) {
      ...
      const { data: existing } = await supabase.from('bookings').select('id').eq('customer_phone', phoneDigits).order('created_at', { ascending: false }).limit(1);
    ...
    const { data } = await supabase.from('bookings').select('*').eq('customer_phone', phoneDigits).neq('service_id', 'verification').in('status', ['confirmed']).order('date');
      ...
      <div className="relative w-full h-[40vh] md:h-[50vh] overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center text-center px-4 pt-8">
          ...
          <div className="flex items-center justify-center gap-6">
            <a href="https://wa.me/972508917748" target="_blank" className="w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center border border-white"><Image src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" width={24} height={24} alt="WA" /></a>
            ...
            <a href="https://waze.com/ul?q=מור 5, אור עקיבא" target="_blank" className="w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center border border-white"><SiWaze size={22} className="text-[#33CCFF]" /></a>
            <a href="https://www.instagram.com/adar_abergel_cosmetics/" target="_blank" className="w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center border border-white"><Instagram size={22} className="text-[#E1306C]" /></a>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-[200] flex items-center justify-center p-4 animate-in fade-in" onClick={() => setShowMyAppointments(false)}>
          <div className="bg-[#FCFBFA] rounded-[3.5rem] shadow-2xl max-w-md w-full max-h-[85vh] overflow-hidden flex flex-col animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white px-10 py-10 flex items-center justify-between border-b border-slate-50">
              <h2 className="text-2xl font-serif italic">התורים שלי</h2>
            ...
            <div className="flex-1 overflow-y-auto p-10">
              {appointmentsNeedsVerification ? (
                <div className="space-y-10 text-center">
                  ...
                  {/* 4 משבצות מעוצבות (זהות לכל האתר) */}
